...
| ScaleEzy's number itself drops | Every ScaleEzy console admin (or only the addresses in the backend's `WHATSAPP_ALERT_EMAILS`) gets an email within a few minutes ("was logged out" or "is not connected"), and a "back" email when it returns; the daily test fails too | Day Books wait until it is relinked: see *Reconnecting a number that logged out* below |
...
How you know: every ScaleEzy console admin gets an email (or only the addresses in the Inventory
backend's `WHATSAPP_ALERT_EMAILS`, if set), **"ScaleEzy's WhatsApp number was logged out"**
